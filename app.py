...
def index():
    ...
    dataGaleri = dataGaleri_collection.find({})
    return render_template('index.html', dataLapangan=dataLapangan, dataGaleri=dataGaleri, dataKontak=dataKontak, dataTentang=dataTentang, dataReview=dataReview)
...
def auth():
    if 'register' in request.form:
        if request.method == 'GET':
            ...
            if not fullname or not phone_number or not email or not password:
                flash('Semua kolom harus diisi', 'danger')
                return redirect(url_for('register'))
            
            if len(password) < 8:
                flash('Kata Sandi minimal 8 karakter', 'danger')
                return redirect(url_for('register'))
            ...
            if not re.match(r'^\+?1?\d{9,15}$', phone_number):
                flash('nomor telepon tidak valid', 'danger')
                return redirect(url_for('register'))
        ...
        if request.method == 'POST':
            ...
            if user is not None and len(user) > 0:
                if bcrypt.checkpw(password.encode('utf-8'), user['password']):
                    ...
                    flash('Email atau kata sandi salah, silahkan coba lagi','danger')
                    return redirect(url_for('register'))
            else:
...
def tambah_data_admin():
    ...
    if request.method == 'POST':
        ...
        password = request.form['password'].encode('utf-8')
        hash_password = bcrypt.hashpw(password, bcrypt.gensalt())  # Konversi ke string
